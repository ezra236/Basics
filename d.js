...
let autoRotateInterval = null; // Interval variable for auto rotation
...
const startAutoRotate = () => {
  if (autoRotateInterval === null) {
    autoRotateInterval = setInterval(() => {
      moveTo += 1; // Adjust the speed of rotation here (higher number means faster)
    }, 10); // Adjust interval time for smoother or faster rotation
  }
};
...
const initEvents = () => {
  ...
  carousel.addEventListener("mouseup", () => {
    ...
    startAutoRotate(); // Resume auto rotation when user stops interacting
  });

  container.addEventListener("mouseleave", () => {
    isMouseDown = false;
    startAutoRotate(); // Resume auto rotation when user leaves the carousel area
  });
